...
class admin_comset_controller extends common{	 
	...
	function savereward_action(){
		...
		if (is_uploaded_file($_FILES['sy_reward_sharelogo']['tmp_name'])) {
				$upload=$this->upload_pic("../data/logo/");
				$logo_path = $this->logo_upload($_FILES['sy_reward_sharelogo'],$upload);
				$_POST['sy_reward_sharelogo'] = $logo_path;
		}
		   foreach($_POST as $key=>$v){
		    	$config=$this->obj->DB_select_num("admin_config","`name`='$key'");
			   if($config==false){
