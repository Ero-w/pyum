...
class user_member_controller extends common{
	...
	function status_action(){
		...
		$this->obj->DB_update_all("resume_expect","`r_status`='".$_POST['status']."'","`uid`='".$uid."' ");
 		...
 		if($this->config['sy_email_lock']=='1' && $_POST['status']==2){
			...
			$this->send_msg_email(array("email"=>$userinfo['email'],'uid'=>$data['uid'],'name'=>$data['name'],"certinfo"=>$_POST['statusbody'],"username"=>$userinfo['username'],"type"=>"lock"));
	...
	function ckstatus_action(){
		...
		if($this->config['sy_email_userstatus']=='1' && $_POST['status']==2){
			...
			$this->send_msg_email(array("email"=>$userinfo['email'],'uid'=>$data['uid'],'name'=>$data['name'],"certinfo"=>$_POST['statusbody'],"username"=>$userinfo['username'],"type"=>"userstatus"));
		}
	...
	function del_action(){
	    ...
	    if($_GET['del'] && !$_GET['send_email'] && !$_GET['send_msg']){
	    	$del=$_GET['del'];
	    	if($del){
				$del_array=array("member","resume","member_statis","look_resume","look_job","resume_show","userid_msg","resume_expect","resume_cert","resume_edu","resume_other","resume_project","resume_skill","resume_training","resume_work","resume_doc","user_resume","down_resume","userid_job","question","msg","attention","rebates","company_msg","fav_job","answer","answer_review","evaluate_log","subscribe","subscriberecord","talent_pool","friend_state"); 
	    		if(is_array($del)){
	    			foreach($del as $k=>$v){
