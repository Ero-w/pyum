...
class job_controller extends common{
	...
	function view_action(){
		...
		$CacheArr=$CacheM->GetCache(array('job','city','hy','com'));
		$job=$JobM->GetComjobOne(array("id"=>(int)$_GET['id'],'state'=>1,'`r_status`<>2 and `status`<>1'));
		...
		$company=$UserinfoM->GetUserinfoOne(array('uid'=>$job['uid']),array('field'=>"`ant_num`,`name`,`pr`,`mun`,`hy`,`linkqq`,`logo`,`address`,`busstops`,`linktel`,`linkman`,`linkphone`,`email_status`,`moblie_status`,`yyzz_status`,`content`,`x`,`y`","usertype"=>2));
		...
		if($_GET['type']){
			if(!$this->uid || !$this->username ){
				...
				if($_GET['type']=='sq'){
					...
					$resume=$ResumeM->SelectExpectOne(array("uid"=>$this->uid,"defaults"=>1,"r_status"=>'1'),"id");
					if(!$resume['id']){
