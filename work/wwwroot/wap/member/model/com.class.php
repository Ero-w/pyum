...
class com_controller extends wap_controller{
	...
	function intergal($type,$statis){
		$data=array("1"=>'会员发布职位用完,购买会员服务更快捷！',"3"=>'会员刷新职位用完！',"4"=>'会员发布猎头职位用完，购买会员服务更快捷！',"6"=>'会员刷新猎头职位用完！',"7"=>'会员发布兼职职位用完，购买会员服务更快捷！',"9"=>'会员刷新兼职职位用完！');
		if($this->config['com_integral_online']=="1"){
	...
	function saveserver_action(){
		...
		if($days<1){
			...
			if($price > $statis['pay'] && $auto==false){
				echo json_encode(array('type'=>2,'msg'=>yun_iconv("gbk","utf-8",'余额不足，请先充值！')));die;
			}else{
				$this->company_invtal($this->uid,$price,$auto,$msg,true,2,'pay',$type);
				if($server=='1'){
	...
	function company_satic(){
		...
		if($statis['vip_etime']>time() || $statis['vip_etime']==0){
			...
			if($this->config['com_integral_online']=='1'){
				$addjobnum=$addpartjobnum=$editjobnum=$editpartjobnum=$addltjobnum='2';
			}else{
				$addjobnum=$addpartjobnum=$editjobnum=$editpartjobnum=$addltjobnum='0';
			}
		}
		$statis['addjobnum']=$addjobnum;
	...
	function wxpay_action(){
		...
		if($_GET['id']){
			...
			if(!empty($order)){
				...
				if(!$this->is_weixin()){
					$jsApiParameters = wxWapOrderMweb(array('body'=>iconv("gbk","utf-8",'充值'),'id'=>$order['order_id'],'url'=>$this->config['sy_weburl'],'total_fee'=>$order['order_price']));
					...
					if($jsApiParameters['mweb_url']){
						...
						if($jsApiParameters['err_code_des']){
							$data['msg']=iconv('utf-8','gbk',$jsApiParameters['err_code_des']);
							
						}elseif($jsApiParameters['return_msg']){
						
							$data['msg']=iconv('utf-8','gbk',$jsApiParameters['return_msg']);
						}else{
							$data['msg']='支付失败';
						}
		...
		if( $statis['vip_etime'] >time() || $statis['vip_etime'] ==0 ){
        	if( $statis['rating_type'] == 1 ){
            	if( $statis['breakjob_num'] > 0 ){
            		...
            		$data['url'] = 'index.php?c=refreshjob&up=' . $jobid;
		...
		if(is_array($rows) && !empty($rows)){
			...
			foreach($rows as $k=>$v){
				...
				foreach($ulist as $val){
					if($v['eid']==$val['id']){
						$rows[$k]['uname']=mb_substr($val['uname'],0,1,'GBK').'**';
						$rows[$k]['edu']=$userclass_name[$val['edu']];
						$rows[$k]['exp']=$userclass_name[$val['exp']];
						if($val['job_classid']){
