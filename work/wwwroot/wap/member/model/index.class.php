...
class index_controller extends wap_controller{
	...
	function resume_action(){
		...
		$rows=$this->obj->DB_select_all("resume_expect","`uid`='".$this->uid."' order by defaults desc","id,name,lastupdate,height_status,open,hits,defaults,statusbody,integrity,hits,statusbody,`topdate`,`is_entrust`,`top`,`r_status`");
		if($rows&&is_array($rows)){
			foreach($rows as $k=>$v){
				$eid[]=$v['id'];
			}
	...
	function rtop_action(){
		...
		if($num>$statis['pay']){
			...
			$result=$this->company_invtal($this->uid,$num,false,'简历置顶',true,1,'pay');
			if($result){
				$time=86400*$days;
